...
def get_depth_values(batch, D, level, device, depth, std, near_far):
    ...
    v_s = cfg.gefu.cas_config.volume_scale[level]
    H, W = int(H * v_s), int(W * v_s)
    if depth is None:
        if cfg.gefu.cas_config.depth_inv[level]:
        ...
        up_scale = cfg.gefu.cas_config.volume_scale[level] / cfg.gefu.cas_config.volume_scale[level-1]
        if up_scale != 1.:
        ...
        if cfg.gefu.cas_config.depth_inv[level-1]:
...
def build_rays_composite(depth, std, batch, training, near_far, level, up_scale=2., xywh=None):
    device = depth.device
    up_scale = cfg.gefu.cas_config.render_scale[level] / cfg.gefu.cas_config.volume_scale[level]
    if up_scale != 1.:
    ...
    x, y, w, h = (xywh * cfg.gefu.cas_config.render_scale[level]).int()
...
def build_feature_volume_composite(feature, batch, D, layers_inter, level, layer_idx, xywh):
    ...
    proj_mats = get_proj_mats(batch, src_scale=cfg.gefu.cas_config.im_feat_scale[level], tar_scale=cfg.gefu.cas_config.volume_scale[level])
...
def build_feature_volume(feature, batch, D, depth, std, near_far, level, tgt_fea, omega_net):
    B, S, C, H, W = feature.shape
    ...
    if tgt_fea is None:
        ...
        tgt_volume = tgt_fea.unsqueeze(2).repeat(1, 1, D, 1, 1)
        tgt_volume = F.interpolate(tgt_volume, size=(D,H,W))
        warped_volumes = None
        volume_context = None
        for s in range(S):
            feature_s = feature[:, s]
            proj_mat = proj_mats[:, s]
            warped_volume_src, _ = homo_warp(feature_s, proj_mat, depth_values)
            warped_volume = (warped_volume_src - tgt_volume).pow_(2)
            warped_volume = warped_volume.permute(0,2,1,3,4).squeeze(0)
            reweight = omega_net(warped_volume)  
            warped_volume = warped_volume.permute(1,0,2,3).unsqueeze(0)
            reweight = reweight.permute(1,0,2,3).unsqueeze(0)
            if warped_volumes is None:
                warped_volumes = (reweight + 1) * warped_volume
                volume_context = (reweight + 1) * warped_volume_src
            else:
                warped_volumes = warped_volumes + (reweight + 1) * warped_volume
                volume_context = volume_context + (reweight + 1) * warped_volume_src
        volume_variance = warped_volumes / S
        volume_context = volume_context / S
        feature_volume = torch.cat((volume_variance,volume_context),dim=1)
...
def raw2outputs(raw, z_vals, white_bkgd=False, fea=None):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        raw: [num_rays, num_samples along ray, 4]. Prediction from model.
        z_vals: [num_rays, num_samples along ray]. Integration time.
    ...
    return {'rgb': rgb_map, 'depth': depth_map, 'weights': weights, 'fea_transmit': fea}
...
def get_img_feat(xyz, img_feat_rgb, batch, training, level):# B * N * S * (11+4)
    ...
    render_scale = cfg.gefu.cas_config.render_scale[level]
...
def parse_layer(layer, batch, hw, level, layer_idx):
    ...
    H, W = hw
    H, W = int(H*cfg.gefu.cas_config.render_scale[level]), int(W*cfg.gefu.cas_config.render_scale[level])
    x, y, w, h = (batch['bbox'][0][layer_idx] * cfg.gefu.cas_config.render_scale[level]).int()
    x, y, w, h = x.item(), y.item(), w.item(), h.item()
