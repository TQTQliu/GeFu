...
class GefuBatchSampler(Sampler):
    def __init__(self, sampler, batch_size, drop_last, sampler_meta):
        self.sampler = sampler
        self.batch_size = batch_size
        self.drop_last = drop_last
        self.input_views = sampler_meta.input_views_num
        self.views_prob = sampler_meta.input_views_prob
        if cfg.fix_random:
            random.seed(0)

    def __iter__(self):
        batch = []
        input_views_num = np.random.choice(self.input_views, 1, p=self.views_prob)
        for idx in self.sampler:
            batch.append((idx, input_views_num.item()))
