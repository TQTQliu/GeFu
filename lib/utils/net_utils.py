...
def load_model(net,
    ...
    pths = [
        ...
        if pth != 'latest.pth' and pth != 'psnr_best.pth' and pth != 'ssim_best.pth' and pth != 'lpips_best.pth'
    ]
    ...
    if 'optim' in pretrained_model:
        # optim.load_state_dict(pretrained_model['optim'])  ###ft
...
def save_model(net, optim, scheduler, recorder, model_dir, epoch, custom=None, last=False):
    ...
    if custom is not None:
        torch.save(model, os.path.join(model_dir, f'{custom}.pth'))
...
def load_network(net, model_dir, resume=True, epoch=-1, custom=None, strict=True):
    if not resume:
        return 0
    if not os.path.exists(model_dir):
    ...
    if os.path.isdir(model_dir):
        ...
        if len(pths) == 0 and 'latest.pth' not in os.listdir(model_dir):
            return 0
        if epoch == -1:
            if custom is not None and custom+'.pth' in os.listdir(model_dir):
                pth = custom
