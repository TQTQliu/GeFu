...
class NetworkWrapper(nn.Module):
    ...
    def forward(self, batch):
        ...
        for i in range(cfg.gefu.cas_config.num):
            ...
            loss += cfg.gefu.cas_config.loss_weight[i] * color_loss
           
            psnr = -10. * torch.log(color_loss) / torch.log(torch.Tensor([10.]).to(color_loss.device))
            scalar_stats.update({f'psnr_{i}': psnr})
          
            num_patchs = cfg.gefu.cas_config.num_patchs[i]
            if cfg.gefu.cas_config.train_img[i]:
                ...
                loss += 0.1 * perceptual_loss * cfg.gefu.cas_config.loss_weight[i]
                scalar_stats.update({f'perceptual_loss_{i}': perceptual_loss.detach()})
                ssim = SSIM(window_size = 7)
                ssim_loss = 1-ssim(inp, tar)
                loss += 0.1 * ssim_loss * cfg.gefu.cas_config.loss_weight[i]
                scalar_stats.update({f'ssim_loss_{i}': ssim_loss.detach()})
                
            elif num_patchs > 0:
                patch_size = cfg.gefu.cas_config.patch_size[i]
                num_rays = cfg.gefu.cas_config.num_rays[i]
                ...
                loss += 0.01 * perceptual_loss * cfg.gefu.cas_config.loss_weight[i]
                scalar_stats.update({f'perceptual_loss_{i}': perceptual_loss.detach()})
